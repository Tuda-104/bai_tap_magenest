...
class SPurchaseOrderLine(models.Model):
    ...
    product_id = fields.Many2one('product.product', string='Sản phẩm', domain=[('purchase_ok', '=', True)],
                                 change_default=True, index='btree_not_null')
    supplier = fields.Char('Nhà cung cấp', store=True, compute='_compute_supplier')
