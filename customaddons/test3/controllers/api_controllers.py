...
class APIControllers(http.Controller):
    @http.route('/test3', type='http', auth='none', methods=["POST"], csrf=False)
    def api_controllers(self, **kw):
        body = json.loads(request.httprequest.data)
        access_token = 'odooneverdie'

        if body["token"] == access_token and body["month"]:
            indicator_evaluation = request.env['indicator.evaluation'].sudo().search([('report_month', '=', body["month"])])
